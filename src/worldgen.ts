    ...
    constructor(noise: Noise.Perlin, chunkX: number, chunkY: number) {
        ...
        for (let y = 0; y < chunkSize; y++) {
            ...
            for (let x = 0; x < chunkSize; x++) {
                const terrain: Terrain.Terrain = genPerlin(noise, chunkX * chunkSize + x, chunkY * chunkSize + y);
                this.land[y].push(terrain);
            }
        }
    }
    get(x: number, y: number): Terrain.Terrain {
        return this.land[y][x];
    }
}

function genPerlin(noise: Noise.Perlin, x: number, y: number): Terrain.Terrain {
